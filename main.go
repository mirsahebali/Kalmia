...
func main() {
	...
	docsRouter.HandleFunc("/documentation/delete", func(w http.ResponseWriter, r *http.Request) {
		fmt.Println("-----------------\nCalled delete handler\n------------------")
		dS.Mu.Lock()
		defer dS.Mu.Unlock()
		startupWg.Add(1)
		handlers.DeleteDocumentation(dS, w, r)
		fmt.Println("-----------------\nResolved delete handler\n------------------")
		startupWg.Done()
	}).Methods("POST")
